...
!! Called by FLASH's Grid unit once per block during startup (after
!! Simulation_init has populated Simulation_data).
...
#include "Eos.h"
   ...
   !! Block index limits in the UNK array
   integer, dimension(LOW:HIGH, MDIM) :: blkLimits    ! interior cells only
   ...
   do k = blkLimitsGC(LOW, KAXIS), blkLimitsGC(HIGH, KAXIS)
      ...
      do j = blkLimitsGC(LOW, JAXIS), blkLimitsGC(HIGH, JAXIS)
         ...
         do i = blkLimitsGC(LOW, IAXIS), blkLimitsGC(HIGH, IAXIS)
            ...
            !!inside the starburst sphere
            inSphere = (dist <= sim_Rsph)

            !! inside the biconical funnel but outside the sphere
